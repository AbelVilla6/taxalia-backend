...
import { ContactMailerConfigError } from './mail.js';
import { ContactSubmissionSchema } from './schemas.js';
function validationErrorCode(issuePath) {
    if (issuePath === 'email')
...
export function buildContactRouter(deps = {}) {
    const app = new Hono();
    app.post('/contact', async (c) => {
        ...
        if (!deps.sendSubmission) {
            return c.json(errorEnvelope(c, 'CONTACT_MAILER_NOT_CONFIGURED', 'Contact form delivery is not configured on this server.'), 503);
        }
        try {
            ...
            return c.json(errorEnvelope(c, 'CONTACT_SEND_FAILED', 'We could not send your message right now. Please try again later.'), 502);
        }
    });
    return app;
}

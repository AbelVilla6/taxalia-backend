...
import { EMPTY_DECISION, OrchestratorDecisionSchema, } from './types.js';
...
export async function route(args) {
    ...
    try {
        ...
        raw = await chatOnceWithTimeout(args.client, req, timeoutMs);
    }
    catch (err) {
        const code = err?.code;
        ...
        warn(`orchestrator:${isTimeout ? 'timeout' : 'chat-failed'} ${errorMessage(err)}`);
    ...
    if (dropped.length > 0) {
        warn(`orchestrator:dropped-unknown-ids requestId=${args.requestId} dropped=[${dropped.join(',')}]`);
    }
    ...
    if (decision.agentsToRun.length === 0) {
        ...
        if (fallback.length > 0) {
            ...
            warn(`orchestrator:keyword-fallback requestId=${args.requestId} selected=[${fallback.join(',')}]`);
 ...
 */
export function keywordFallback(userMessage, agents) {
 ...
 */
async function chatOnceWithTimeout(client, req, timeoutMs) {
    let timer;
    const timeoutPromise = new Promise((_, reject) => {
        timer = setTimeout(() => {
            const e = new Error(`orchestrator chatOnce exceeded ${timeoutMs}ms`);

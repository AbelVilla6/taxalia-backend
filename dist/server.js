...
import { createLogger, getDefaultLogger } from './observability/logger.js';
...
function createCorsGuard(allowlist, logger) {
    return async (c, next) => {
...
export function createApp(env, registry = createArtifactRegistry()) {
    const app = new Hono();
    ...
    try {
        ...
        logger.info({ dbPath: env.BLOG_DB_PATH, seeded }, 'blog content store ready');
        // Public read API.
        ...
        app.route('/api/admin', buildAdminRouter({
            repo,
            auth,
...
async function main() {
    ...
    if (env.SKIP_OLLAMA_CHECK) {
        logger.warn('SKIP_OLLAMA_CHECK is set: starting without Ollama. Chat will fail until Ollama is reachable; blog API is unaffected.');
    }
    try {
        if (!env.SKIP_OLLAMA_CHECK)
            await client.checkModel();
        ...
        if (code === 'MODEL_MISSING') {
            logger.fatal({ model: env.OLLAMA_MODEL }, `MODEL_MISSING: ${env.OLLAMA_MODEL} not found. Run 'npm run setup'.`);
        }
        else if (code === 'OLLAMA_UNREACHABLE') {
            logger.fatal({ host: env.OLLAMA_HOST }, 'OLLAMA_UNREACHABLE: cannot reach Ollama. Is the server running?');
        }
        else {

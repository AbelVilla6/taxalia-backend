 ...
 */
export function openBlogDb(dbPath) {
...
export function ensureBlogSchema(db) {
  ...
  `);
    db.prepare(`UPDATE posts
     ...
     WHERE translation_group_id = '' OR translation_group_id IS NULL`).run();
    const groups = db
        .prepare(`SELECT translation_group_id AS id, MAX(draft) AS has_draft
       FROM posts
       WHERE translation_group_id IS NOT NULL AND translation_group_id != ''
       GROUP BY translation_group_id`)
        .all();
    const insertGroup = db.prepare(`INSERT OR IGNORE INTO translation_groups (id, published)
     VALUES (?, ?)`);

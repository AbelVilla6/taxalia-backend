...
const TRANSLATE_SKILL_PATH = join(dirname(fileURLToPath(import.meta.url)), '../editorial/translate-post.md');
 ...
 */
export function buildAdminRouter(deps) {
    ...
    const requireFreshPassword = async (c, next) => {
    ...
    app.post('/password', requireAuth, async (c) => {
        ...
        const ok = auth.changePassword(c.get('username'), parsed.data.currentPassword, parsed.data.newPassword);
        if (!ok) {
            return c.json({ error: 'INVALID_CREDENTIALS' }, 401);
        }
        return c.json({ ok: true });
    ...
    app.post('/translate', requireAuth, requireFreshPassword, async (c) => {
        if (!deps.ollama) {
        ...
        try {
            ...
            return c.json({ error: 'OLLAMA_ERROR', detail: err instanceof Error ? err.message : String(err) }, 502);
        }
        let candidate;
        try {
        ...
        const jsonLd = t.jsonLd == null ? null : typeof t.jsonLd === 'string' ? t.jsonLd : JSON.stringify(t.jsonLd);
        return c.json({
            post: {
                lang: targetLang,
                title: t.title,
    ...
    app.post('/preview', requireAuth, requireFreshPassword, async (c) => {
        const body = (await c.req.json().catch(() => null));
        const markdown = typeof body?.markdown === 'string' ? body.markdown : '';
        return c.json({ html: renderPostHtml(markdown).html });
    });
    app.get('/posts', requireAuth, requireFreshPassword, (c) => {
        return c.json({ posts: repo.listAll() });
    });
    app.get('/posts/:id', requireAuth, requireFreshPassword, (c) => {
    ...
    app.post('/posts', requireAuth, requireFreshPassword, async (c) => {
    ...
    app.put('/posts/:id', requireAuth, requireFreshPassword, async (c) => {
        const id = Number(c.req.param('id'));
        const parsed = PostSchema.safeParse(await c.req.json().catch(() => null));
        if (!parsed.success) {
            return c.json({ error: 'INVALID_BODY', issues: parsed.error.issues }, 400);
    ...
    app.delete('/posts/:id', requireAuth, requireFreshPassword, (c) => {
        const id = Number(c.req.param('id'));
    ...
    app.post('/media', requireAuth, requireFreshPassword, async (c) => {
